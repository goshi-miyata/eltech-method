# エルテクメソッド

👉 https://goshi-miyata.github.io/eltech-method/

## 概要
本ドキュメントは当社におけるコーディング指針をまとめたものです。

目的は以下の通りです。
- コード品質の向上
- 可読性の統一
- 技術者の基礎能力底上げ

## 前提
- 言語はJavaをベースとしています。
- 実案件ではプロジェクト規約を優先します。
- 思想に偏りがあるかもしれませんが、それは許容しています。なぜなら、絶対的な正解があるのであればFormatterにチェックさせるだけでよいためです。

## 基本思想
美しいコードとは「可読性の高いコード」である。

## 使い方
- 実装時の指針として利用する。
- コードレビューの判断基準として利用する。

## 運用ルール
本リポジトリでは、改善提案はPull Requestで受け付けています。
mainブランチへの直接更新は禁止です。
詳細は以下を参照してください：
- [運用ルール](./docs/contributing.md)

## 📚 目次

### 🧠 基本原則
- [可読性至上主義](./principles/readability.md)

### 🏗 設計
- [構造化](./design/structure.md)
- [責務分離](./design/method-responsibility.md)

### 💻 コーディング
- [returnルール](./coding/return.md)
- [三項演算子](./coding/ternary.md)
- [ifコメント](./coding/if-comment.md)
- [範囲条件](./coding/range-if.md)

### 🏷 命名
- [命名規則](./naming/naming-conventions.md)

### ❌ アンチパターン
- [一覧](./anti-patterns/anti-patterns.md)

### ✅ レビュー
- [チェックリスト](./checklist/review.md)


